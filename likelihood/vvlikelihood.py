...
import gpytorch
from gpytorch.likelihoods import MultitaskGaussianLikelihood, _MultitaskGaussianLikelihoodBase, _GaussianLikelihoodBase,FixedNoiseGaussianLikelihood
...
def logbarrierFunction(x, low, high, c):
    out = Tensor([0.])
    n = x.shape[0]
    m = x.shape[1]
    zero_tensor = Tensor([0.])
...
#     print(n)
    for i in range(n):
        for j in range(m):
            try:
                out = out  -(torch.log(-x[i,j] + low))  
            except:
                out = out + c
            try:
                out = out   -(torch.log(x[i,j] - high))
            except:
                out = out + c
...
def barrierFunction_bounds(x, bounds, c):
    out = Tensor([0.])
    n = x.shape[0]
    m = x.shape[1]
    # print(m)
...
class TadFixedNoiseGaussianLikelihood(FixedNoiseGaussianLikelihood):

    def __init__(self, noises, **kwargs):
        
        super().__init__(noises)
        
    def get_ell(self, agg_data, f_target,x_, g_theta1, model, noise_value, g_theta2, h,box): #, cov_noise1, cov_noise2):
...
#        g_theta2 = Tensor(g_theta2.reshape(math.ceil(g_theta2.shape[0]/g_theta1.shape[1]), g_theta1.shape[1]))
        ...
        cov_noise2 =  noise_value * torch.eye(g_theta2.shape[0])
#
        ...
        C11 = K.forward(g_theta1, g_theta1, add_jitter = True) + cov_noise1.to(device)
        ...
        C22 = K.forward(g_theta2, g_theta2, add_jitter = True) + cov_noise2.to(device)
       ...
       # noise1_vec = torch.sqrt(cov_noise1.diag())
        ...
        C21 = C12.t()
        #with gpytorch.settings.cholesky_jitter(jitter):
        Q21 = C22 - gpytorch.inv_matmul(C11, C12, C21)
...
#
       ...
       # with gpytorch.settings.cholesky_jitter(jitter):
        Qf1 = Cff - gpytorch.inv_matmul(C11, C1f, Cf1) #
...
#        print('end T')
       ...
       # print(barrierFunction(x_, -3., 3., 1000.)  -  barrierFunction(g_theta2, -3., 3., 1000.))
        ell = ell -  barrierFunction_bounds(x_, box, 1000000.)  -  barrierFunction_bounds(g_theta2, box, 1000000.)
...
#        print(pf1 - f_target)
    ...
    def get_ell_no2(self, agg_data, f_target,x_, g_theta1, model, noise_value, h,box): #, cov_noise1, cov_noise2):
    #def get_ell(self, agg_data, f_target, g_theta1, model, likelihood, noise_value, samples): #, cov_noise1, cov_noise2):
...
#
       ...
       # Qf12 =  Qf12.add_jitter(1e-8)
        ...
        try:
            #with gpytorch.settings.cholesky_jitter(self.custom_jitter):
            inv_quad_Qf1 = gpytorch.inv_matmul(Qf1, f_target - pf1,  (f_target - pf1).t())
        except:
...
#        print('end T')
        ...
        ell = -1./2. * (  logdet_Qf1 + inv_quad_Qf1) # +  trace_term)
    
        #print(barrierFunction(x_, -3., 3., 1000.)  -  barrierFunction(g_theta2, -3., 3., 1000.))
        ell = ell -  barrierFunction_bounds(x_, box, 10000.) # -  barrierFunction_bounds(g_theta2, box, 10000.)
        
        
#        print(pf1 - f_target)
        return ell, pf1, Qf1,logdet_Qf1 +  inv_quad_Qf1
...
#        print('end T')
        ...
        ell = ell - barrierFunction(x_, -3., 3., 100000.)  -  barrierFunction(g_theta2, -3., 3., 100000.) # - barrierFunction(x_, 0.01, 0.95, 1000000.)  -  barrierFunction(g_theta2, 0.01, 0.95, 1000000.)
        
        
#        print(pf1 - f_target)
        return ell, pf1, Qf1, Qf12, logdet_Qf12 +  inv_quad_Qf12, Q21

    def get_ell_(self, agg_data, f_target,x_, g_theta1, model, noise_value, g_theta2, h,box,N): #, cov_noise1, cov_noise2):
    #def get_ell(self, agg_data, f_target, g_theta1, model, likelihood, noise_value, samples): #, cov_noise1, cov_noise2):
...
#        diff = diff.reshape(diff.shape[0], 1)
    ...
    def get_pf12(self, Q21,g_theta1, g_theta2, x, data2, pf1,p21, model, noise_value,num_tasks=2):

        cov_noise =  noise_value * torch.eye(num_tasks * g_theta1.shape[0])  #find CHANGE 3 to N = numtasks   #likelihood._shaped_noise_covar([ agg_data.shape[0],
        #cov_noise2 =  noise_value * torch.eye(2 * g_theta2.shape[0])
        device = g_theta1.device
...
#         print(K.forward(g_theta1, g_theta1, add_jitter = True).shape)
    ...
    def get_L2method(self, agg_data,f_target,x_, g_theta1, model, likelihood,cov_noise1):

      #find CHANGE 3 to N = numtasks   #likelihood._shaped_noise_covar([ agg_data.shape[0],
        #cov_noise2 =  noise_value * torch.eye(2 * g_theta2.shape[0])
        mu = model.mean_module
        K = model.covar_module
       ...
       # noise1_vec = torch.sqrt(cov_noise1.diag())
        ...
        inv_quad = torch.linalg.norm(pf1 - f_target)

        
       
        diag_trace_term_arg = Qf1.diag()
...
#        print('end T')
        ...
        L2 = -1./2. * (  inv_quad +  trace_term)
    
      
        
        L2 = L2 - barrierFunction(x_, -3., 3., 100000.)#barrierFunction(x_, 0.01, 0.95, 1000000.)  -  barrierFunction(g_theta2, 0.01, 0.95, 1000000.)
        
        
        
        
        return L2, pf1, Qf1, inv_quad, trace_term
